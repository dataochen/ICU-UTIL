...
public class ThreadFastErrorUtil {
    ...
    public static ThreadFastErrorUtil getInstance(ExecutorService executor) {
        if (null == instance) {
    ...
    /**
     * 如果自定义了线程池 优先用自定义的 否则使用默认的TTL
     * 如果要使用MemUtil 必须使用TTL线程池
     *
     * @param executor
     */
    private ThreadFastErrorUtil(ExecutorService executor) {
        if (executor == null) {
            executor = new ThreadPoolExecutor(10, 10, 2000,
                    TimeUnit.MILLISECONDS, new ArrayBlockingQueue<Runnable>(1024),
                    new FastErrorUtilThreadFactory(), (r, e) -> {
                throw new RejectedExecutionException("ThreadFastErrorUtilTask " + r.toString() +
                        " rejected from " +
                        e.toString());
            }
            );
...
//        ExecutorCompletionService 先完成的线程会保存在ExecutorCompletionService中的队列中 如果只使用普通的线程池执行AbstractExecutorService.submit 会在第一个线程阻塞等待Future的结果 不满足需求
...
//        都成功
     ...
     * @return
     */
    public static FastErrorResponse success() {
        return new FastErrorResponse(true, null);
    }
